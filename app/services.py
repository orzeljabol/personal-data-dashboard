...
def get_last_7_days_entries(db: Session = Depends(get_db)):
    ...
    return entries
def calculate_average_sleep_hours(db: Session):
    entries = get_last_7_days_entries(db)
    total_sleep = 0
...
def calculate_average_mood(db: Session):
    entries = get_last_7_days_entries(db)
    total_mood = 0
    for entry in entries:
    ...
    if entries:
        ...
        return 0
    return average_mood
def calculate_average_energy(db: Session):
    entries = get_last_7_days_entries(db)
    total_energy = 0
    for entry in entries:
...
def calculate_total_deep_work_minutes(db: Session):
    entries = get_last_7_days_entries(db)
    total_deep_work = 0
    for entry in entries:
...
def calculate_average_deep_work_minutes(db: Session):
    entries = get_last_7_days_entries(db)
    total_deep_work = 0
    for entry in entries:
        if entry.deep_work_minutes is not None:
...
def calculate_average_exercise_minutes(db: Session):
    entries = get_last_7_days_entries(db)
    total_exercise = 0
    for entry in entries:
...
def calculate_total_exercise_minutes(db: Session):
    entries = get_last_7_days_entries(db)
    total_exercise = 0
    for entry in entries:
        if entry.exercise_minutes is not None:
...
def calculate_average_stimulation_minutes(db: Session):
    entries = get_last_7_days_entries(db)
    total_stimulation = 0
    for entry in entries:
...
def calculate_total_stimulation_minutes(db: Session):
    entries = get_last_7_days_entries(db)
    total_stimulation = 0
    for entry in entries:
        if entry.stimulation_minutes is not None:
...
def calculate_average_litres_water(db: Session):
    entries = get_last_7_days_entries(db)
    total_water = 0
    for entry in entries:
